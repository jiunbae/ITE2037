...
/**
 * 가장 체력이 적은 감염체를 위해 곧바로 투신하는 샘플 플레이어입니다.
 * 
 * 생존자 이동: '방향 섞기' 기능으로 만들어 둔 순서에 따라 4턴을 한 주기로 두고 반복적으로 비슷한 자리를 맴돕니다.
 * 감염체 이동: 현 위치에 시체가 있는 경우 주변 칸으로 이동하며 그렇지 않은 경우 정화 기도를 올립니다.
 * 영혼 배치: 기준 체력을 10으로 두고 각 감염체의 현재 체력과 기준 체력의 차이(최소 1)의 합이 가장 큰 칸으로 배치합니다. (대부분의 경우 배치 즉시 사망합니다)
 * 직접 감염: ID가 짝수인 경우 수락합니다. 그렇지 않은 경우 거절합니다.
 * 
 * @author Racin
 *
 */
public class SamplePlayer_Paramedic extends Player
{
	/**
	 ...
	 */
	public SamplePlayer_Paramedic(int ID)
	{
		name = String.format("구급대원#%d", ID);
	...
	public Point Soul_Spawn()
	{
		 ...
		 * 구급대원의 영혼 배치:
		 * 
		 * 1. 각 감염체의 치유 필요도(체력이 10보다 작은 경우 10 - 체력, 그렇지 않은 경우 1)를 감염체가 있는 칸에 기록
		 * 2. 기록된 치유 필요도가 가장 큰 위치들 중 기점과 가장 가까운 첫번째 위치로 배치
		 */
		Point pointToSpawn = basePoint;

		//각 칸의 치유 필요도를 기록하기 위한 배열 사용
		...
		for ( PlayerInfo other : othersInfo_withinSight )
		{
			...
			if ( other.GetState() == PlayerInfo.State.Infected )
			{
				...
				//치유 필요도: 감염체의 체력이 10 미만인 경우 10 - 체력, 그렇지 않은 경우 1
				...
				//최대값이 바뀌었다면 '치유 필요도가 가장 큰 칸' 목록 초기화
				...
				//현재 칸의 값이 최대값과 같다면 현재 칸을 '치유 필요도가 가장 큰 칸' 목록에 추가 (위에서 최대값이 바뀌었다면 항상 추가됨)
				if ( weights[pos_other.y][pos_other.x] == max_weight )
					list_pos_max_weight.add(pos_other);
			}
		}
		
		//검사가 끝나면 치유 필요도가 가장 큰 칸 목록에서 '기점 설정' 기능으로 정해 둔 기점과 가장 가까운 첫번째 칸 선택 - 최소 하나 존재
