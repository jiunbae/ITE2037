 ...
 *
 */
public class Presenter_Mode4 extends GameFrame
{
	...
	/**
	 * BS 키를 눌렀는지(게임 종료 시점까지 자동으로 진행하고 있는지) 여부를 나타냅니다.
	 */
	boolean isSkipping;
	...
	/**
	 * 리플레이의 대상이 될 플레이어의 ID입니다.
	 */
	int ID_playerToFocus = 0;
	
	public Presenter_Mode4(GameFrameSettings gfs, Classroom_Settings cs)
	{
		super(gfs);
		...
		{
			if ( cs.custom_player_classes.get(iPlayer) == cs.playerToFocus )
			{
	...
	void UpdateCellInfo()
	{
		if ( state == PresenterStateCode.Waiting_Enter )
		{
			...
			if ( found == true )
			{
				CellInfo info = classroom.players[ID_playerToFocus].cells[iRow][iColumn];
				row_focused = iRow;
